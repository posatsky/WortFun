...
export const GameBoard: React.FC<GameBoardProps> = ({
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="bg-white border-2 border-amber-200/80 rounded-3xl p-4 sm:p-6 mb-6 shadow-md">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="relative shrink-0">
              <select
                ...
                className="appearance-none bg-orange-500 text-white font-black px-4 py-2.5 pr-9 rounded-2xl border-2 border-orange-400 hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-orange-400 cursor-pointer text-sm shadow-md"
              >
                {levels.map((lvl, idx) => {
          ...
          <div className="flex items-center gap-4 sm:gap-6 bg-amber-50 px-5 py-2.5 rounded-2xl border-2 border-amber-200/80 shrink-0 text-xs sm:text-sm font-bold">
            <div>
              <div className="text-slate-400 text-[10px] uppercase font-extrabold tracking-wider">Залишилось</div>
            ...
            <div>
              <div className="text-slate-400 text-[10px] uppercase font-extrabold tracking-wider">Час</div>
              <div className="font-mono font-black text-orange-600 text-base">
            ...
            <div>
              <div className="text-slate-400 text-[10px] uppercase font-extrabold tracking-wider">Серія</div>
              <div className={`font-black text-base flex items-center gap-1 ${stats.streak > 1 ? 'text-orange-600 animate-pulse' : 'text-slate-700'}`}>
        ...
        <div className="mt-4 pt-3 border-t border-amber-100 flex items-center gap-3">
          <div className="flex-1 bg-slate-200 h-4 rounded-full overflow-hidden border-2 border-white shadow-inner">
            <motion.div
      ...
      {/* CARDS GRID */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-5 min-h-[380px]">
        <AnimatePresence mode="popLayout">
          {cards
            .filter((card) => !card.isMatched)
            .map((card) => {
              let cardStyle = "bg-white rounded-3xl shadow-[0_8px_0_0_#cbd5e1] border-2 border-slate-200 hover:border-orange-400 text-slate-800 hover:shadow-[0_8px_0_0_#fb923c]";
              let stateBadge = null;

              if (card.isCorrect) {
                cardStyle = "bg-emerald-500 rounded-3xl shadow-[0_8px_0_0_#059669] text-white border-4 border-emerald-300 scale-105 transition-all z-10";
                stateBadge = (
                  <span className="flex items-center gap-1 text-[11px] font-black bg-white text-emerald-800 px-2.5 py-0.5 rounded-full shadow-sm animate-bounce">
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                    <span>Вірно!</span>
                  </span>
                );
              } else if (card.isError) {
                cardStyle = "bg-rose-500 rounded-3xl shadow-[0_8px_0_0_#be123c] text-white border-4 border-rose-300 animate-pulse transition-all z-10";
                stateBadge = (
                  <span className="flex items-center gap-1 text-[11px] font-black bg-white text-rose-800 px-2.5 py-0.5 rounded-full shadow-sm">
                ...
                cardStyle = card.language === 'de'
                  ? "bg-indigo-600 rounded-3xl shadow-[0_8px_0_0_#4338ca] text-white border-4 border-indigo-300 scale-[1.02]"
                  : "bg-orange-500 rounded-3xl shadow-[0_8px_0_0_#c2410c] text-white border-4 border-orange-300 scale-[1.02]";
              }
              ...
              return (
                <motion.button
                  ...
                  whileTap={{ scale: 0.96 }}
                  className={`relative rounded-3xl p-4 sm:p-5 flex flex-col justify-between text-left transition-all duration-150 min-h-[105px] sm:min-h-[120px] group cursor-pointer active:translate-y-1 active:shadow-none ${cardStyle}`}
                >
                  ...
                  {/* Main Word Text */}
                  <div className="font-extrabold text-base sm:text-lg md:text-xl leading-tight mt-auto">
                    {card.text}
                  </div>
      ...
      <div className="mt-8 flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-3xl border-2 border-amber-200 text-xs font-semibold text-slate-600 shadow-sm">
        <div className="flex items-center gap-2">
